                ...
                <table>
                    ...
                    <tbody>
                        ${paginatedTasks.map(task => {
                            ...
                            return `
                                <tr style=\"cursor: pointer;\" onclick=\"viewTaskDetail('${task.id}')\">
                                    <td onclick=\"event.stopPropagation()\">
                                        <input type=\"checkbox\" ${task.status === 'done' ? 'checked' : ''}
                                            onchange=\"window.toggleTaskStatus('${task.id}', this.checked)\"
                                            style=\"width:16px;height:16px;cursor:pointer;accent-color:var(--primary-purple);\"
                                        >
                                    </td>
                                    <td>
                                        <span style=\"${task.status === 'done' ? 'text-decoration:line-through;opacity:0.5;' : ''}\">${task.title}</span>
                                    </td>
                        ...
                        <div class=\"form-row\">
                            ...
                            <div class=\"form-group\">
                                ...
                                <select name=\"assignee\">
                                    ...
                                    <option value=\"Joe\" ${task.assignee === 'Joe' ? 'selected' : ''}>Joe</option>
                                    <option value=\"Malak\" ${task.assignee === 'Malak' ? 'selected' : ''}>Malak</option>
...
window.viewTaskDetail = function(taskId) {
    ...
    const linkedClient = linkedProject ? window.crmState.clients.find(c => c.id === linkedProject.clientId) : null;

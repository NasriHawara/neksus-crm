...
function renderProjectsPage() {
    ...
    content.innerHTML = `
                    ...
                    <tbody>
                        ${paginatedProjects.map(project => `
                            <tr>
                                ...
                                <td>${window.getClientName(project.clientId) || '-'}</td>
        ...
        <style>
            ...
            .task-card, .finance-record-card, .client-card {
                background: var(--bg-darker);
                border: 1px solid var(--border-color);
                border-radius: 10px;
                padding: 16px;
            ...
            .task-card:hover, .finance-record-card:hover, .client-card:hover {
                border-color: var(--primary-purple);
                transform: translateY(-2px);
...
function filterAndSortProjects() {
    ...
    if (projectFilters.search) {
        ...
        projects = projects.filter(project => {
            const clientName = window.getClientName(project.clientId);
...
window.exportProjects = function() {
    const projects = window.crmState.projects.map(project => ({
        name: project.name,
        client: window.getClientName(project.clientId) || '',
...
window.editProject = function(projectId) {
    ...
    const clientOptions = clients.map(c =>
        `<option value=\"${c.id}\" ${c.id === project.clientId ? 'selected' : ''}>${c.name}</option>`
    ).join('');
...
window.viewProjectDetail = function(projectId) {
    ...
    // Get linked client
    const linkedClient = window.crmState.clients.find(c => c.id === project.clientId);

    // Get linked tasks (tasks with this projectId)
    const linkedTasks = window.crmState.tasks.filter(task => task.projectId === projectId);

    // Get linked finance transactions (income/expenses tied to this project)
    const linkedFinances = (window.crmState.finances || []).filter(f => f.projectId === projectId);
    ...
    const projectIncome = linkedFinances.filter(f => f.type === 'income').reduce((sum, f) => sum + (parseFloat(f.amount) || 0), 0);
    const projectExpenses = linkedFinances.filter(f => f.type === 'expense').reduce((sum, f) => sum + (parseFloat(f.amount) || 0), 0);
        ...
        <div class=\"modal active\" id=\"projectDetailModal\" style=\"z-index: 1001;\">
            <div class=\"modal-content\" style=\"max-width: 900px;\">
                <div class=\"modal-header\">
                    <div>
                        ...
                        <p style=\"color: var(--text-secondary); font-size: 14px; margin-top: 4px;\">
                            ${linkedClient ? `Client: ${linkedClient.name}` : 'No client assigned'}
                        </p>
                    </div>
                ...
                <div class=\"detail-tabs\">
                    ...
                    <button class=\"detail-tab\" onclick=\"switchProjectTab(event, 'finances')\">
                        <svg width=\"16\" height=\"16\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" style=\"display: inline; margin-right: 6px;\">
                ...
                <div class=\"modal-body\" style=\"max-height: 600px; overflow-y: auto;\">
                    ...
                    <div class=\"detail-tab-content active\" id=\"overview-tab\">
                        ...
                        <div class=\"stats-row\">
                            ...
                            <div class=\"stat-box\">
                                <div class=\"stat-number\" style=\"color: var(--success);\">$${projectIncome.toFixed(2)}</div>
                                <div class=\"stat-label\">Income</div>
                            </div>
                            <div class=\"stat-box\">
                                <div class=\"stat-number\" style=\"color: var(--danger);\">$${projectExpenses.toFixed(2)}</div>
                                <div class=\"stat-label\">Expenses</div>
                            </div>
                            <div class=\"stat-box\">
                                <div class=\"stat-number\" style=\"color: ${projectProfit >= 0 ? 'var(--success)' : 'var(--danger)'};\">${projectProfit >= 0 ? '+' : ''}$${projectProfit.toFixed(2)}</div>
                        ...
                        <div class=\"info-grid\">
                            ...
                            <div class=\"info-item\">
                                ...
                                <span class=\"info-value\">${linkedClient ? linkedClient.name : 'Not assigned'}</span>
                            </div>
                            <div class=\"info-item\">
                    ...
                    <div class=\"detail-tab-content\" id=\"finances-tab\">
                                ...
                                <div style=\"display: flex; justify-content: space-between; align-items: start; margin-bottom: 12px;\">
                                    <div>
                                        <h4 style=\"font-size: 16px; font-weight: 600; color: var(--text-primary); margin-bottom: 4px;\">
                                            ${finance.description || finance.category || 'Transaction'}
                                        </h4>
                                        <p style=\"font-size: 13px; color: var(--text-secondary);\">
                                            ${window.getClientName(finance.clientId) || 'No client'}
                                        </p>
                                    </div>
                                    <div style=\"text-align: right;\">
                                        <div style=\"font-size: 20px; font-weight: 700; color: ${finance.type === 'income' ? 'var(--success)' : 'var(--danger)'}; margin-bottom: 4px;\">
                                            ${finance.type === 'income' ? '+' : '-'}$${(parseFloat(finance.amount) || 0).toFixed(2)}
                                        </div>
                                        <span class=\"badge ${finance.status === 'unpaid' ? 'danger' : 'success'}\">
                        ...
                        `).join('') : `
                            <div class=\"empty-section\">
                                ...
                                <p>No income or expenses linked to this project yet.</p>
                            </div>
                        `}
                    </div>

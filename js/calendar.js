            ...
            html += `
                <div class=\"day-event-card\" style=\"border-left: 4px solid ${getEventColor(event.type)};\" onclick=\"viewEventDetail('${event.id}')\" data-testid=\"event-card-${event.id}\">
                    ...
                    <div class=\"event-card-content\">
                        ...
                        ${event.projectId ? `<span class=\"badge info\">${window.getProjectName(event.projectId)}</span>` : ''}
                        ${event.clientId ? `<span class=\"badge success\">${window.getClientName(event.clientId)}</span>` : ''}
                    </div>
                </div>
            `;
        });
                    ...
                    <div style=\"display: grid; gap: 16px; margin-bottom: 20px;\">
                        ...
                        ${event.projectId ? `
                            <div style=\"background: var(--bg-secondary, #f3f4f6); padding: 12px; border-radius: 8px;\">
                                ...
                                <div style=\"font-size: 14px; font-weight: 500;\">${window.getProjectName(event.projectId)}</div>
                            </div>
                        ` : ''}
                        ...
                        ${event.clientId ? `
                            <div style=\"background: var(--bg-secondary, #f3f4f6); padding: 12px; border-radius: 8px;\">
                                ...
                                <div style=\"font-size: 14px; font-weight: 500;\">${window.getClientName(event.clientId)}</div>
                            </div>
                        ` : ''}
                    </div>
...
window.editEvent = function(eventId) {
    ...
    const projectOptions = projects.map(p => 
        `<option value=\"${p.id}\" ${p.id === event.projectId ? 'selected' : ''}>${p.name}</option>`
    ).join('');
    const clientOptions = clients.map(c => 
        `<option value=\"${c.id}\" ${c.id === event.clientId ? 'selected' : ''}>${c.name}</option>`
    ).join('');

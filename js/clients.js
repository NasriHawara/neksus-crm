...
window.viewClientDetail = function(clientId) {
    ...
    // Get linked projects (projects where clientId matches this client's id)
    const linkedProjects = window.crmState.projects.filter(p => p.clientId === clientId);
    ...
    // Get linked finance transactions (income/expenses tied directly to this client)
    const linkedFinances = (window.crmState.finances || []).filter(f => f.clientId === clientId);
    const clientIncome = linkedFinances.filter(f => f.type === 'income').reduce((sum, f) => sum + (parseFloat(f.amount) || 0), 0);
    const clientExpenses = linkedFinances.filter(f => f.type === 'expense').reduce((sum, f) => sum + (parseFloat(f.amount) || 0), 0);
    const clientProfit = clientIncome - clientExpenses;
    const clientUnpaid = linkedFinances.filter(f => f.status === 'unpaid').reduce((sum, f) => sum + (parseFloat(f.amount) || 0), 0);

    // Get linked retainers
    const linkedRetainers = (window.crmState.retainers || []).filter(r => r.clientId === clientId);
    const activeRetainer = linkedRetainers.find(r => r.status === 'active');
        ...
        <div class=\"modal active\" id=\"clientDetailModal\" style=\"z-index: 1001;\">
            <div class=\"modal-content\" style=\"max-width: 900px;\">
                ...
                <div class=\"detail-tabs\">
                    ...
                    <button class=\"detail-tab\" onclick=\"switchDetailTab(event, 'finances')\">
                        <svg width=\"16\" height=\"16\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" style=\"display: inline; margin-right: 6px;\">
                            <line x1=\"12\" y1=\"1\" x2=\"12\" y2=\"23\"/>
                            <path d=\"M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6\"/>
                        </svg>
                        Finances (${linkedFinances.length})
                    </button>
                ...
                <div class=\"modal-body\" style=\"max-height: 600px; overflow-y: auto;\">
                    ...
                    <!-- Finances Tab -->
                    <div class=\"detail-tab-content\" id=\"finances-tab\">
                        <div class=\"section-title\">
                            <svg width=\"20\" height=\"20\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\">
                                <line x1=\"12\" y1=\"1\" x2=\"12\" y2=\"23\"/>
                                <path d=\"M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6\"/>
                        ...
                        <div class=\"info-grid\" style=\"margin-bottom: 24px;\">
                            <div class=\"info-item\">
                                ...
                                <span class=\"info-value\" style=\"color: var(--success);\">$${clientIncome.toFixed(2)}</span>
                            </div>
                            <div class=\"info-item\">
                                <span class=\"info-label\">Total Expenses</span>
                                <span class=\"info-value\" style=\"color: var(--danger);\">$${clientExpenses.toFixed(2)}</span>
                            </div>
                            <div class=\"info-item\">
                                <span class=\"info-label\">Net Profit</span>
                                <span class=\"info-value\" style=\"color: ${clientProfit >= 0 ? 'var(--success)' : 'var(--danger)'};\">${clientProfit >= 0 ? '+' : ''}$${clientProfit.toFixed(2)}</span>
                            </div>
                            <div class=\"info-item\">
                                <span class=\"info-label\">Unpaid</span>
                                <span class=\"info-value\" style=\"color: ${clientUnpaid > 0 ? 'var(--danger)' : 'var(--text-primary)'};\">$${clientUnpaid.toFixed(2)}</span>
                            </div>
                        </div>

                        ${activeRetainer ? `
                            <div style=\"background: var(--bg-darker); border: 1px solid var(--border-color); border-radius: 8px; padding: 16px; margin-bottom: 24px; display: flex; justify-content: space-between; align-items: center;\">
                                <div>
                                    <p style=\"font-weight: 600; color: var(--text-primary); margin-bottom: 4px;\">Active Retainer</p>
                                    <p style=\"font-size: 13px; color: var(--text-secondary);\">$${parseFloat(activeRetainer.amount).toFixed(2)}/month · Billed on day ${activeRetainer.billingDay || 1}</p>
                                </div>
                        ...
                        ${linkedFinances.length > 0 ? linkedFinances.sort((a, b) => new Date(b.date) - new Date(a.date)).map(finance => `
                            <div class=\"finance-record-card\">
                                <div style=\"display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;\">
                                    <div>
                                        <h4 style=\"font-size: 15px; font-weight: 600; color: var(--text-primary);\">${finance.description || finance.category || 'Transaction'}</h4>
                                        <p style=\"font-size: 12px; color: var(--text-secondary); margin-top: 2px;\">${finance.date ? window.formatDate(finance.date) : ''} ${finance.projectId ? '· ' + (window.getProjectName(finance.projectId) || '') : ''}</p>
                                    </div>
                                    <div style=\"text-align: right;\">
                                        <div style=\"font-size: 16px; font-weight: 700; color: ${finance.type === 'income' ? 'var(--success)' : 'var(--danger)'};\">
                                            ${finance.type === 'income' ? '+' : '-'}$${(parseFloat(finance.amount) || 0).toFixed(2)}
                                        </div>
                                        <span class=\"badge ${finance.status === 'unpaid' ? 'danger' : 'success'}\" style=\"font-size: 11px;\">${finance.status === 'unpaid' ? 'Unpaid' : 'Paid'}</span>
                                    </div>
                                </div>
                            </div>
                        `).join('') : `
                            <div class=\"empty-section\">
                                ...
                                <p>No income or expenses linked to this client yet.</p>
                            </div>
                        `}
                    </div>

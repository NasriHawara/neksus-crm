...
function setupRealtimeSync() {
    ...
    unsubscribeFunctions.contractors = onSnapshot(
        collection(db, 'contractors'),
        (snapshot) => {
            window.crmState.contractors = snapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data()
            }));
            refreshCurrentPage();
        ...
        (error) => {
            ...
            showNotification('Error syncing contractors', 'error');
        }
    );

    // Real-time listener for retainers
    unsubscribeFunctions.retainers = onSnapshot(
        collection(db, 'retainers'),
        (snapshot) => {
            window.crmState.retainers = snapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data()
            }));
            refreshCurrentPage();
        ...
        (error) => {
            console.error('Error syncing retainers:', error);
            showNotification('Error syncing retainers', 'error');
        }
    );
...
function initNavigation() {
    ...
    navItems.forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            if (!page) return; // no data-page (e.g. external link) -> let browser handle it normally
            e.preventDefault();
            navigateTo(page);
        });
    });
...
function navigateTo(page) {
    ...
    switch(page) {
        ...
        case 'tasks':
            ...
            break;
        case 'contractors':
            if (typeof window.loadContractorsPage === 'function') {
                window.loadContractorsPage();
            }
            break;
        case 'retainers':
            if (typeof window.loadRetainersPage === 'function') {
                window.loadRetainersPage();
            }
            break;
        case 'calendar':
...
async function loadAllData() {
    try {
        ...
        window.crmState.contractors = contractorsSnapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data()
        }));

        // Load retainers
        const retainersSnapshot = await getDocs(collection(db, 'retainers'));
        window.crmState.retainers = retainersSnapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data()
        }));
...
function generateRecentActivities() {
    const activities = [];
    const { clients, projects, tasks, events, finances } = window.crmState;
    ...
    (finances || []).forEach(finance => {
        if (finance.createdAt) {
            activities.push({
                type: 'finance',
                action: 'created',
                title: `${finance.type === 'income' ? 'Income' : finance.type === 'expense' ? 'Expense' : 'Transaction'} added: ${finance.description || finance.category || 'N/A'}`,
                description: `Amount: ${formatCurrency(parseFloat(finance.amount) || 0)}`,
                timestamp: finance.createdAt,
                timestampMs: parseTimestamp(finance.createdAt),
                icon: 'file',
                color: finance.type === 'income' ? 'green' : 'orange'
            });
        }
        if (finance.updatedAt && finance.updatedAt !== finance.createdAt) {
            activities.push({
                type: 'finance',
                action: 'updated',
                title: `Transaction updated: ${finance.description || finance.category || 'N/A'}`,
                description: `Status: ${finance.status || 'N/A'}`,
                timestamp: finance.updatedAt,
                timestampMs: parseTimestamp(finance.updatedAt),
                icon: 'file',
                color: 'orange'
            });
        }
...
function loadDashboard() {
    ...
    const { clients, projects, tasks, activities } = window.crmState;

    // Calculate stats
    const totalClients = clients.length;
    const activeProjects = projects.filter(p => p.status === 'active').length;
    const unpaidPayments = (window.crmState.finances || []).filter(f => f.status === 'unpaid');
    const unpaidAmount = unpaidPayments.reduce((sum, f) => sum + (parseFloat(f.amount) || 0), 0);
    const thisMonth = (window.crmState.finances || [])
        .filter(f => {
            const fDate = new Date(f.date);
            const now = new Date();
            return fDate.getMonth() === now.getMonth() && fDate.getFullYear() === now.getFullYear() && f.type === 'income';
        })
...
function performSearch(event) {
    ...
    (finances || []).forEach(finance => {
        if ((finance.description && finance.description.toLowerCase().includes(searchTerm)) ||
            (finance.category && finance.category.toLowerCase().includes(searchTerm))) {
            results.push({ type: 'Transaction', name: finance.description || finance.category, data: finance });
        }
    });
